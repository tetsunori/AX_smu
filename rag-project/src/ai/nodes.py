...
2. 'database' - 전국 지역별 인구, GRDP, 인구 증감률, 지역 경제 비교 등 통계 데이터베이스 조회가 필요한 질문
   예: "2022년 인구가 가장 많은 지역은?", "지역별 1인당 명목 GRDP를 알려줘", "경기도와 충남의 인구와 GRDP를 비교해줘"

3. 'vector' - 업로드된 문서의 내용 검색이 필요한 질문
   예: "문서에 나온 인구 정책을 알려줘", "보고서의 지역 경제 전망을 요약해줘"
...
예시:
- 이전: "2022년 경기도 인구는?" → 현재: "충남은?" → 재구성: "2022년 충청남도 인구는?"
- 이전: "지역별 GRDP를 알려줘" → 현재: "인구와 비교해줘" → 재구성: "지역별 GRDP와 인구를 같은 연도 기준으로 비교해줘"
...
def generate_answer(state: AgentState) -> AgentState:
    ...
    system_prompt = f"""
당신은 전국 인구 및 지역내총생산(GRDP) 통계 분석 전문가입니다.
...
- 사용한 연도, 지역, 인구 및 GRDP 수치를 명확히 포함하세요
- GRDP는 명목(당해년가격)과 실질(기준년가격)을 구분하세요
- 인구와 GRDP를 비교할 때는 동일한 연도 기준인지 확인하세요
- 불필요한 전제 조건이나 한계를 언급하지 말고, 질문에 직접 답변하세요
- 정보가 정말로 없는 경우에만 "해당 정보를 찾을 수 없습니다"라고 말하세요
- 통계 결과를 이해하기 쉬운 친절하고 자연스러운 어조로 답변하세요
